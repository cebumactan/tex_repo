...
\begin{align*}
 ...
 u_t&=\sigma_{xx},
\end{align*}
where we find $h(\theta)$ an increasing function of $\theta$. The velocity gradient $u$ grows at the point where the stress is less than the surrounding. The size of the stress is determined by the viscosity and the $u$ itself. The increment of temperature diminishes the stress, whereas the increment of $u$ increases the stress and the overall size is determined from this competition. Suppose the decay rate of the viscosity is sufficiently fast, which is not ``orderly'', so that the temperature effect overwhelm the other, or the region with higher temperature has the smaller stress size. This increases $u$ and in turn, results in the increment of the temperature again and the positive feedback is conceivably triggered. 

\cite{KOT14} considered the viscosity with exponential law
$$ \mu(\theta) = e^{-\alpha\theta}$$
and constructed a family of solutions that exhibit localizing instability. In the course, the authors introduced the series of techniques that exploits the scale invariance property of the system \eqref{intro:system}. They worked on the whole space $x \in \mathbb{R}$ and seeked  the self-similar solutions. This technique turns out to be successful in the plastic shear flow of solid with power-law type stress \cite{KLT_2016, LT16}, namely
$$ \sigma_p = \sigma_p(\gamma,u) = \gamma^{-m}u^n, \quad \text{where $\gamma$ is the shear strain and $m,n>0$.}$$




It is our objective to construct 
\begin{equation*}
\begin{aligned}
...
\begin{equation}
 \lambda = \frac{D}{2+2n}\frac{U(0)^{1+n}}{\Theta(0)^{1+\alpha}} - \frac{2}{2+2n}. \label{intro:lambda}
\end{equation}
\begin{equation} \label{intro:restriction}
...
In the regime localization takes place, instead of speaking of stability we speak of non-arbitrariness of the instability. See the discussion on the competition between the Hadamard instability and the diffusion in \cite{KOT14}. Even though the field variables localizes, developing singularity as $t \rightarrow \infty$, the intermediate process is maintained to be smooth and oscillation is suppressed. In regard, 

the m, temperature effect give a dissipative mechanism. Other than that,  With heat conduction, its one effect gives a dissipative mechanism. Other than this mechanism, 


flow of non-newtonian fluid.
\begin{enumerate}
 \item rectlinear(?) non-newtonian viscous flow
 \item decreasing viscosity, power law, arrehnius law, exponential law
 \item kinematics, initial-boundary value problem
 \item energy, temperature increases to infinity
 \item uniform shear
 \item self-similar localization in whole space
 \item strain independent solid, identical equation
 \item oscillation is suppressed
 \item temperature localization, velocity gradient localization
\end{enumerate}
...
$$ \sigma = \sigma(\theta,u) = \theta^{-\alpha}u^n.$$
Then the first equation \eqref{eq:g} drops out from the system, and we focus on the system
\begin{equation} \label{eq:strsof}
 \begin{aligned}
  v_t &= \sigma_x,\\
  \theta_t &= \sigma u,\\
  \sigma &=\theta^{-\alpha}u^n.
