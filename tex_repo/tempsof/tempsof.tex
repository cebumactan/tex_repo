...
\end{align*}
The goal of this section is to reveal the linear stability around the four equilibrium points delivering a perspective on the behaviors of orbits.
\footnotetext[1]{
...
In the below, we denote the three eigenvalues of the equilibrium point $M_i$ by $\mu_{ij}$, $j=1,2,3$ and the three eigenvectors by $X_{ij}$, $j=1,2,3$.\medskip
...
\end{align*}
Along $X_{01}$, as $q$ increases, $p$ stays the same but $r$ decreases. Along $X_{02}$, as $q$ increases, $p$ increases as well but $r$ decreases. Note that the third eigenvalue along $X_{03}$ has much greater value than other two because $n \ll 1$.
\medskip
...
\begin{align*}
 ...
 &\mu_{21} =-1-\frac{1+\alpha}{\lambda(1+n)} c_0 -d_1, \quad \mu_{22}=-1, \quad \mu_{23} = -\frac{\alpha-n}{n(1+n)}\lambda r_1^{1+n}
\end{align*}
\medskip

\noindent{\bf Equilibrium $M_0$ is a saddle.}
\begin{align*}
 &X_{31} = (1,0,0) \quad X_{32}=(0,1,0), \quad X_{33}=(0,0,1),\\
 &\mu_{31} =-\frac{1+\alpha}{\lambda(1+n)} c_0 -d_1, \quad \mu_{22}=-1, \quad \mu_{23} = \frac{1}{n}\Big(-\frac{\alpha-n}{\lambda(1+n)} c_0 +d_1\Big)
\end{align*}
