...
\end{aligned}
\end{equation*}
interpreting $\big(\Gamma(\xi),V(\xi),\Theta(\xi),\Sigma(\xi),U(\xi)\big)$ as the initial states that give rise to the localization that is self-similar parametrized by $U(0)$ and $\Theta(0)$ as time proceeds. 

\subsubsection{Initial non-uniformities}
We view $\big(\Gamma(\xi),V(\xi),\Theta(\xi),\Sigma(\xi),U(\xi)\big)$ as initial non-uniformities or perturbations to the uniform shearing states. By investigating asymptotic behavior of the heteroclinic orbit as $\eta \rightarrow \pm\infty$, we 

\begin{proposition}
 Let $\big(\Gamma(\xi),V(\xi),\Theta(\xi),\Sigma(\xi),U(\xi)\big)$ be a smooth self-similar profile satisfying \eqref{eq:continuity} and let $\varphi^\star(\eta)=\big(p(\eta),q(\eta),r(\eta)\big)$ be the associated variables defined by \eqref{eq:tvars} and \eqref{eq:ratios}. Suppose $\varphi^\star(\eta)$ is the heteroclinic orbit of \eqref{eq:pqrsystem} that connects $M_0$ to $M_1$. Then, 
 \begin{enumerate}
...
%   where the translation factor $\eta_0$ of $\varphi^\star(\eta)$ and $\bG_0$ are related by \eqref{eq:eta0}.
  ...
  \begin{aligned}
    \Gamma(\xi) &= \Gamma(0) + O(\xi^2), &V(\xi) &= U(0)\xi + O(\xi^3), & \Theta(\xi) &= \Theta(0) + O(\xi^2),\\
    \Sigma(\xi) &= \Theta(0)^{-\alpha}{U(0)^n}+ O(\xi^2), & U(\xi) &= U(0) + O(\xi^2).
  \end{aligned}
  \end{equation}
  \item[(iii)] The asymptotic behavior as $\xi \rightarrow \infty$ is given by
  \begin{equation} \label{eq:ss_asymp1}
  \begin{aligned}
    V(\xi) &= O(1), &    G(\xi) &= O(\xi^{- \frac{1}{m-n}}),\\
	S(\xi) &= O(\xi), &   U(\xi) &= O(\xi^{- \frac{1}{m-n}}).
  \end{aligned}
  \end{equation}
 \end{enumerate}
