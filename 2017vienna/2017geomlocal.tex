...
\institute{\vskip 5pt joint with Thodoros Katsaounis\quad and \quad Athanasios Tzavaras\\KAUST, SA}
\date{~}
\begin{frame}
  \titlepage
  \vskip -60pt
...
%  &\lim_{\tau \rightarrow \infty} C_3 x_3(\tau) \exp\left(C_4n\l^2 \tau^{\frac{1+\alpha}{1+m}}\right)\tau^{-\chi_3} = p_3, \quad \text{for some $\chi_i$, $i=1,2,3$}.
...
%     \subfigure{\includegraphics[width=3cm]{pqplane.eps}}
...
% 
%  
%  {\bf Corollay 3.}  For $n>0$ sufficiently small, $\mathcal{F}(N_2;M_0(n))$ and $W^{s}(M_1(n))$ again intersect transversally, forming the heteroclinic orbit.
% %The most rapidly escaping orbit $\phi^\star(\eta)\subset W^u(M_0)$ intersects $W^s(M_1)$ transversally.
%    \begin{figure}
%   \centering
%     \psfrag{p}{\scriptsize$p$}
%     \psfrag{q}{\scriptsize$q$}
