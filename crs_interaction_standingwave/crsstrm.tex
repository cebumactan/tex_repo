...
1. Let $B \subset E$ be the ball with {\blue $\|(u,v)\| \le k^{-\frac{m-1}{2}}$}. We first show that for $k$ sufficiently large and $R \in \left[\frac{m-\epsilon}{2\pi}k\log k, \frac{m+\epsilon}{2\pi}k\log k\right]$, $F(B) \subset B$. Let $(\bar u, \bar v) := F(u,v)$. Then,
 ...
 \end{align*}
% By Lemma \ref{inter}, $U_0U_i \le Ce^{-R} = Ck^{-\frac{m}{2\pi}k} \le Ck^{-\frac{m}{2} - \sigma}$ for some $\sigma>0$ if $k$ is large. Also, 
% $$\|u^2\|_{L^2} + \|u^3\|_{L^2} + \|uv\|_{L^2} +\|uv^2\|_{L^2} \le Ck^{-\frac{m}{2} - \sigma'} \quad \text{for some $\sigma'>0$}.$$
% Therefore for sufficiently small $|\beta|$ and sufficiently large $k$, $\| \bar u\|_0 \le k^{-\frac{m}{2}}$. Nextly,
 \begin{align*}
...
% We have that
% \begin{align*}
%  \left\|\left(\sum U_i\right)^3 - \sum U_i^3\right\|_{L^2} &= \bigg\|\sum_{\substack{(i,j,\ell) \in \{1,\cdots,n\}^3 \\ \text{\scriptsize not $i=j=\ell$}}} U_iU_jU_\ell\bigg\|_{L^2} \le C\|U_1\|_{L^2} \|U_1U_2\|_\infty \le Ck^{-\frac{m}{2}-\sigma''},
% \end{align*} 
%%  \begin{align*}
%%   \left|\left(\sum U_i\right)^3 - \sum U_i^3\right| &= \sum_{\substack{(i,j,\ell) \in \{1,\cdots,n\}^3 \\ \text{\scriptsize not $i=j=\ell$}}} U_iU_jU_\ell \le CU_1^2U_2 \\
%%   &\le CU_1e^{-2R\sin\left(\frac{\pi}{k}\right)} \le CU_1k^{-\frac{m}{2}-\sigma''}
%%  \end{align*}
% where we have used the Lemma \ref{inter} and that 
% $$2R\sin\left(\frac{\pi}{k}\right) \ge \frac{mk}{\pi}\sin\left(\frac{\pi}{k}\right)\log k \ge(\frac{m}{2}+ \sigma'') \log k \quad \text{for some small $\sigma''>0$ if $k$ is large.}$$
