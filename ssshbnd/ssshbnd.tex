...
\begin{aligned}
 ...
 \theta_t &= \tau v_x \quad \text{(energy conservation)}	\\%\label{eq:th}\\
...
The constitutive law $\tau = \theta^{-\alpha}\gamma^m u^n$ characterizes the material property in the form of the power law: $\alpha>0$ measures the degree of the thermal softening, $m$ measures that of strain hardening (or softening for the plastic flow), and $n>0$ is the rate sensitivity presumably very small that accounts for the non-newtonian viscosity. This model is the simple paradigm to capture the shear localization and has been studied in mechanical literature REF and mathematical literature as well REF. Such an empirical power law $\eqref{intro-system0}_4$ can be found in REF in the context of buckling problem.
...
\end{aligned}
\end{equation}
where the exponents $a$, $b$, $c$, and $d$ are selected as in \eqref{eq:exponents} according to the scale invariance property and $\lambda>0$ accounts for the rate of localization. The self-similar variable $\xi=(t+1)^\lambda x$. One should notice that the scale exponent $\lambda$ is taken in opposite sign to the one in the typical parabolic problems where the defocussing behavior is expected. 

The model \eqref{intro-system0} admits the {\it uniform shearing solution} as a universal one regardless of the precise exponents $(\alpha,m,n)$. For a given constant $\gamma_0$ and the initial temperature $\theta_0(x)$, it consists of
\begin{equation} \label{intro:uss}
 ...
 \end{aligned}
\end{equation}
This is a solution that are contrasted with those exhibit localizing instability. In the latter, the growth of strain is superlinear and in the same time localizes in a narrow zone. 

Widely accepted explanation REF why such a localizing instability can take place finds the following positive feedback scenario for the reason: the material keeps loading and 
this mechanical working causes the increases of temperature. Due to the imperfectness of the deformation, the small non-uniformities in strain presences and this results in the non-uniformities in the heat production. When the speed of deformation is so high such that the time needed for the heat to diffuse out is not sufficient, then the process becomes effectively {\it adiabatic}, i.e., the heat produced at a spot does not diffuse out but accumulates. Metals typically thermally softens and strain hardens. For such a metal where the thermal softening outweighs the hardening response, the net response is softening and the spots that had larger deformation than surrounding becomes even easier to deform. This, in turn, gives rise to the severer non-uniformities in strain, triggering the positive feedback mechanism.

We conceive the instability of \eqref{intro:uss} by the loss of hyperbolicity in the system. For $n=0$, \eqref{intro-system0} is a first order system and we illustrate in Appendix \ref{append:hadamard} that along the uniform shearing solution, the hyperbolicity is lost if $-\alpha+m<0$. This foresees the catastrophic growth of oscillations in the initial value problem, which has coined the term {\it Hadamard Instability}.

As pointed out in REF, the Hadamard Instability cannot be the complete accounts on the shear band phenomena. This is because what is observed in the process is the orderly development of localization rather than the arbitrary growth of oscillations. As an opposing mechanism to the Hadamard Instability, the small viscosity ($n>0$) is suggested in the model, and the system is of parabolic-hyperbolic type. We should also note that, if $n$ is sufficiently large, then diffusive mechanism takes over so that the localization is suppressed, or \eqref{intro:uss} is expected to be globally asymptotically stable. 

We are led to focus on the regime $-\alpha+m+n<0$ for the localization to take place, where the Hadamard Instability and the diffusive mechanism comparably effects. This coincides with the range indicated from the study of the linearized problem. REF

The first half of the tackling the problem is to give a proper formulation. The series of techniques and transformations for this to be done were first introduced in REF, and were successfully adapted to the strain independent model ($\tau=\varphi(\theta)u^n$) REF, and to the temperature independent model ($\tau=\varphi(\gamma)u^n$) REF. Due to self-similarity ansatz \eqref{intro-sols}, problem reduces to solving the ordinary differential equations
\begin{equation} \label{intro:ss-odes}
\begin{aligned}
 a \Gamma(\xi) + \lambda \xi \Gamma'(\xi) &= U(\xi), &
 b V(\xi) + \lambda \xi V'(\xi) &= \Sigma'(\xi), & \Sigma(\xi) &= \Theta(\xi)^{-\alpha} \Gamma(\xi)^m U(\xi)^n,\\
 c \Theta(\xi) + \lambda \xi \Theta'(\xi)&=\Sigma(\xi) U(\xi),& V'(\xi)&=U(\xi).
\end{aligned} 
\end{equation}
Solution is sought subject to the boundary conditions
\begin{equation}
 V(0)=U'(0)=\Gamma'(0)=\Sigma'(0)=\Theta'(0)=0 \label{intro:bdry0}
\end{equation}
whose meaning is detailed in Section \ref{sec:char}.
The coefficient $\xi$ in the highest order terms in \eqref{intro:ss-odes} makes system singular at $\xi=0$ and not autonomous. General existence theory is not available in this singular odes and for its study case-by-case analyzes are required. For \eqref{intro:ss-odes}, this singularity is resolved by introducing series of nonlinear transformations. In its final form, we arrive at the following four-dimensional dynamical system on $(p,q,r,s)$
\begin{equation}\label{intro:slow}
 ...
 \end{aligned}
\end{equation}
parametrized by $(\lambda,\alpha,m,n)$. The problem looking for solutions of \eqref{intro:ss-odes} is turned into that looking for solutions of \eqref{intro:slow}. In doing so, \eqref{intro:bdry0} are transmitted to the corresponding asymptotic conditions for \eqref{intro:slow}.

Throughout our study, two key points pertains originated from the smallness of $n$. First, in terms of the original variables \eqref{eq:vars}, the last equation $\eqref{intro-system0}_4$ dictates that the problem is singularly perturbed. At $n=0$, it specifies an algebraic equation restricting solutions in a manifold $\tau=\theta^{-\alpha}\gamma^m$. This reduction is jeopardized once $n$ becomes positive and certain layers possibly forms according to the imposed conditions on the solution. It turns out that the asymptotic conditions imposed on the solutions dictates that the solutions resides {on a \it slow manifold} 

Secondly, $\eqref{intro-system0}_3$ is re-written as
$$ \partial_t\Big(\frac{\theta^{1+\alpha}}{1+\alpha}\Big) = \partial_t\Big(\frac{\gamma^{1+m}}{1+m}\Big)\gamma_t^n$$
and at $n=0$, it says that the temperature and the strain (up to the powers) are tied in their evolution. Thus, modulo initial data one variable is expressed by the other. When $n>0$, while we cannot remove one of the variable, its co-evolving feature is expected to persist. This feature in regard with the smallness of $n$ is regularly perturbed, and we will see the corresponding analysis in our discussion in Section \ref{sec:proof}.

It is interesting to see how those two differently perturbed aspects manifest in \eqref{intro:slow}. $n$ in the left-hand-side of $\eqref{intro:slow}_3$ indicates the presence of the {\it fast-slow} structure. In association with the first aspect, we carry out the Chapman-Enskog type reduction via geometric singular perturbation theory, reducing the the dimensions from four to three. This cope with the singularly perturbed nature. In association with the second, we seek for the manifold that specifies $\theta$ in terms of $\gamma$ that would reduce the dimensions from three to two. For instance when $n=0$, it is the surface where $\frac{\gamma^{1+m}}{\theta^{1+\alpha}} = \frac{1+m}{1+\alpha}$. The way of thinking in this line is that the flow would be essentially of two dimensional that is perturbed from the case $n=0$ but in two different ways.

However, the implementation of above program is technically subtle. In this type of reduction, we speak of {\it normally hyperbolicity} of the manifold from where the perturbative manifold is constructed. The two dimensional surface turns out not to be normally hyperbolic, and we implement the second reduction via global phase space analysis. As a matter of fact, the latter is merely homeomorphic to the critical manifold, which is in accordance with that the manifold is not normally hyperbolic. This will be detailed in Section \ref{sec:proof}.
