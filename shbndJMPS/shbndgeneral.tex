...
% \section{Factorization of $A(t)$ of the linearized problem}
% In this section, we apply the results developed in the preceding section on our linearized problem. We compute the absolutely continuous factorization explicitly: $(i)$ In case $n>0$, we show the diagonalization is absolutely continuous; $(ii)$ in case $n=0$, we show the upper triangularization is absolutely continuous.
% 
% \subsection{$n=0$}
% We are studying non-autonomous system $x'=A(\tau)x$, with coefficient
% $$ A(\tau) =
%    \begin{pmatrix}
%    0 & 0 & -\l^2\\
%    0 & 0 & 0\\
%    0 & 0 & 0\\
%   \end{pmatrix} + \frac{1}{\tau}
%    \begin{pmatrix}
%    0 & 0 & 0\\
...
%    m\frac{-\alpha+m}{1+m} &
%    -(1+\alpha)
%   \end{pmatrix}= A_\infty + \frac{1}{\tau}A_1(\tau)$$
% for $k\ge1$. The latter term decays to $0$ but not in integrable way. 
% 
% It is clear that diagonalization cannot be continuous for $A_\infty$ has triple eigenvalue $0$ whereas its geometric multiplicity is merely $2$. More precisely, for finite $\tau$, all three eigenvalues are distinct but collapse down to a single value $0$ as $\tau \rightarrow \infty$.
% the eigenvectors associated to $\lambda_1$, $\lambda_2$, and $\lambda_3$ are respectively
% \begin{align*}
...
% 
% 
% The next best possibility is a Jordan canonical
% 
% $n=0$ case
% 
% Jordan block calculation
% 
% 
% 
...
%
% \begin{theorem}
% Suppose that the diagonal entries of $U(t)$ are $L^1_{loc}(a,\infty)$, off-diagonal entries are $L^1(a,\infty)$ and $\mathcal{E}(t)$ is a matrix with entries $\mathcal{E}_{ij}(t)$ that is integrable, i.e.,
...
%  \begin{align*}
%   y' &= U(t)y, \quad y(a)\in \underset{\ell\in L}{ \textrm{Span}}\{\hat{\ell}\}, \quad \text{and writes}\\
%   (x-y)' &= U(t)(x-y) + \mathcal{E}(x-y) + \mathcal{E}y.
%  \end{align*}
% Component-wisely,
% \begin{align*}
...
% Opening on the relative perturbation.
...
%    \frac{1}{\tau}\Big( n+ \frac{-\alpha+m}{1+m}\Big) +\kappa(\tau)n\l^2&
%    \frac{1}{\tau}\Big(m\frac{-\alpha+m}{1+m}\Big) + \kappa(\tau)m\l^2&
%    -\frac{1}{\tau}(1+\alpha) -(n+\kappa(\tau))\l^2 
%   \end{pmatrix}
...
% 
% 
% In the below, we put $\kappa=0$. Then
% % 
% % 
% % From the fourth equation in \eqref{eq:linsystem}, we remove one of the variable. We remove $\Theta$ and get three linear equations
% \begin{equation}
%  \begin{aligned}
%   \partial_\tau U &= \Sigma_{xx},\\
%   \partial_\tau\Gamma &= \frac{1}{\tau}\frac{1+\alpha}{1+m}(U-\Gamma),\\
...
% \end{equation}
% 
%  For each $k$-th mode, we collect terms in the matrix form $\dot{X}_k = A_k(\tau)X_k$. 
% At $k$-th mode $k\ge1$,
% \begin{align}
%   \frac{d}{d\tau}
%   \begin{pmatrix}
...
\begin{theorem}[non resonant case] \label{thm:1} Assume \eqref{eq:nodoubleroot}. $\chi_1$ and $\chi_2$ are independent of $\ell$ and their signs bifurcate by $q=-\alpha+m+n$:
 ...
 \end{enumerate}
\end{theorem}
% For cases \eqref{eq:nodoubleroot} does not hold, two eigenvalues $\lambda_1(\tau)$ and $\lambda_2(\tau)$ differs only by order strictly higher than $\frac{1}{\tau}$. As long as linear stability is concerned, this raises resonance in the long time asymptotics. The following exposition helps us state our theorem concise. By being marginally stable we meant every solution is bounded but there is a solution $x(\tau)$ that does not decay; by being stable that every solution decays to $0$; by being unstable that there is a solution $x(\tau)$ that grows (at polynomial order). 
% \begin{theorem}[resonant case] \label{thm:2}
%  \begin{enumerate}
%   \item If $\alpha=m$, then the uniform shear is marginally stable.
%   \item If $\alpha\ne m$ but $(-\alpha+m)(1+n)^2 = 4nm(1+\alpha)$, then uniform shear is stable if $m>0$; marginally stable if $m=0$; and is unstable if $m<0$.
%  \end{enumerate}
% \end{theorem}
% \begin{theorem}[resonant case]
%  \begin{enumerate}
%   \item If $\alpha=m$, then the uniform shear is marginally stable: we find a solution $x(\tau)$ that is bounded but not decaying.
%   \item If $\alpha\ne m$ but $(-\alpha+m)(1+n)^2 = 4nm(1+\alpha)$, then $(i)$ uniform shear is stable if $m>0$: Every solution decays to $0$; $(ii)$ uniform shear is marginally stable if $m=0$: We find a solution $x(\tau)$ that is bounded but not decaying; $(iii)$ uniform shear is saddle if $m<0$: We find a solution $x(\tau)$ that grows (at polynomial order).
%  \end{enumerate}
% \end{theorem}
...
% \begin{proposition}
%  Suppose $(\alpha,m,n)$ is such that \eqref{eq:auxquad} has a double root $z_1(0)=z_2(0)=a$. Then for $\epsilon>0$ small enough, $\exists C$ such that two roots of \eqref{eq:reduced_poly} exist in $|z-a| \le C\sqrt{\epsilon}$.
% \end{proposition}
% \begin{remark}
%  The conclusion of above proposition is that two eigenvalues still found near to $a$ and differ by strictly higher order than $\mathcal{O}\left(\tau^{-1}\right)$. Seen from the linear stability of non-autonomous system in Appendix, this difference is of integrable order and does not affect the long time behavior. Hence we may say the system effectively has a double eigenvalue.
% \end{remark}
% \begin{proof}
...
% \end{proof}
% 
% Since $\mathbf{N}\Big(A(\tau)-\lambda_1(\tau)\Big)$ and  $\mathbf{N}\Big(A(\tau)-\lambda_2(\tau)\Big)$ merge up in the limit, we have to find the block factorization as we did in the inviscid case. With the same reasoning we had, every element of $\mathbf{N}\Big(A-\lambda_1\Big)$ restricted in $ \mathbf{N}\Big(\big(A-\lambda_1\big)\big(A-\lambda_2\big)\Big)$ is in the range of $A-\lambda_2$ and there must be a $\tilde s_2(\tau)$ such that
% $$ (A-\lambda_2\big)\tilde s_2 = s_1.$$
% The difficulty is that we only know that $\lambda_i(\tau)$, $i=1,2,3$ are continuous in $\tau$ and they are roots of \eqref{eq:poly}; in this resonant cases we do not know if eigenvalues admit analytic expansions but know $|z_i - a| \le C\frac{1}{\sqrt{\tau}}$. Upon checking assumptions of Proposition \ref{prop:stab}, this lack of explicit formulas makes our arguments abstract. Instead of doing this, we make uses of that $|z_i - a| \le C\frac{1}{\sqrt{\tau}}$ and pick basis vectors that are nearly in the kernel.
% 
% We set $\bar s_1(\epsilon) = \begin{pmatrix} a +\frac{1+\alpha}{1+m} \\ \frac{1+\alpha}{1+m} \\ -\frac{\epsilon a\big(a+ \frac{1+\alpha}{1+m}\big)}{\l^2} \end{pmatrix}$. Let $R$ be a generic quantity that is of order $o(\tau)$ as $\tau \rightarrow \infty$, i.e., integrable. Then we find that 
% \begin{align*}
%  \frac{1}{\tau}|z_i-a| =R, \quad |\bar s_1 - s_1| = (z_1-a)\begin{pmatrix} 1\\0\\0 \end{pmatrix} + R, \quad |A(\bar s_1 - s_1)| = R,
% \end{align*}
% and thus we have
% $$A\bar s_1 = \frac{a}{\tau} \bar s_1 + R, \quad A\hat s_2 = \frac{a}{\tau} \hat s_2 + \bar s_1.$$
% 
% 
% and pick $\hat s_2$ such that $\big(A-a)\hat s_2 = \bar s_1$.
% 
%  \begin{align*}
%   y' = \begin{pmatrix} -\frac{a}{\tau} & 1 & 0\\ 0 & -\frac{a}{\tau} & 0 \\ 0 & 0 & -n\l^2 + \frac{\lambda_3'(0)}{\tau} \end{pmatrix}y + R(\tau)y,
%  \end{align*}
% 
% \begin{proof}[Proof of Theorem \ref{thm:2}]
%  We first consider $-\alpha+m=0$. If this is the case, then $a=-1$.
% $(1,\frac{1}{\tau})$ is a solution of the system with semi-group part only and this is the maximum growth. By Proposition \ref{prop:stab}, we conclude that the uniform shear is marginally stable. 
% 
% If $\alpha\ne m$ but $(-\alpha+m)(1+n)^2 = 4nm(1+\alpha)$ then $a>1$ if $m>0$; $a=1$ if $m=0$; $a<1$ if $m<0$. If $a>1$, then estimate in Proposition \ref{prop:tri-stab} gives the stability; if $a=1$ the same argument above gives the marginal stability; if $a<1$ then $\Big(\tau^{1-a}, \tau^{-a}\Big)$ is a solution of the system with semi-group part only.
% \end{proof}
% 
%  
%  
...
% %$$ \frac{1}{\tau}z_1(0), \quad \frac{1}{\tau}z_2(0), \quad \text{and} \quad -n\l^2+ \frac{1}{\tau}\lambda_3'(0).$$
% \begin{proposition} \label{prop:fund}
%  Let $n>0$, \eqref{eq:nodoubleroot} holds. Let $p_i$ $i=1,2,3$ be the three constant eigenvectors of $\displaystyle S_\infty \triangleq  \lim_{\tau \rightarrow \infty} S(\tau)$ of \eqref{eq:order1diag}. Then the linearized system \eqref{eq:l-system} for $k\ge1$ has the three decaying modes described respectively by solutions $\varphi_1(\tau;k)$, $\varphi_2(\tau;k)$, and $\varphi_3(\tau;k)$ satisfying the asymptotic behaviors in the below. 
%  for some $\tau_0$.
% % $\begin{bmatrix} | & | & | \\
...
%
% \begin{align*}
% %  \lambda_1(\tau) &= - \frac{1}{\tau}\frac{1+\alpha}{2(1+m)} + i\frac{1}{\sqrt{\tau}}\l\sqrt{\frac{-\alpha+m}{1+m}}\sqrt{\Big(1-\frac{1}{\tau} \frac{(1+\alpha)^2}{4\l^2(-\alpha+m)(1+m)}\Big)},\\
% %  \lambda_2(\epsilon) &= - \frac{1}{\tau}\frac{1+\alpha}{2(1+m)} - i\frac{1}{\sqrt{\tau}}\l\sqrt{\frac{-\alpha+m}{1+m}}\sqrt{\Big(1-\frac{1}{\tau} \frac{(1+\alpha)^2}{4\l^2(-\alpha+m)(1+m)}\Big)},\\
...
% \noindent{\bf subcase 3 $q=-\alpha+m<0$ for $\tau$ large:}
% \begin{align*}
%  \lambda_1(\epsilon) &= - \epsilon\frac{1+\alpha}{2(1+m)} + \sqrt{\epsilon}\l\sqrt{\frac{\alpha-m}{1+m}}\sqrt{\Big(1-\epsilon \frac{(1+\alpha)^2}{4\l^2(-\alpha+m)(1+m)}\Big)},\\
%  \lambda_2(\epsilon) &= - \epsilon\frac{1+\alpha}{2(1+m)} - \sqrt{\epsilon}\l\sqrt{\frac{\alpha-m}{1+m}}\sqrt{\Big(1-\epsilon \frac{(1+\alpha)^2}{4\l^2(-\alpha+m)(1+m)}\Big)},\\
...
% \end{aligned}
% \end{equation}
% $\beta(\tau)$ and $\gamma(\tau)$ are both $\mathcal{O}\big(\frac{1}{\tau}\big)$ as $\tau \rightarrow \infty$. If $-\alpha+m=0$ then $\gamma \equiv 0$
%
%
% where the square root is understood as the branch of nonnegative imaginary part.
% \begin{align*}
%  \lambda_1(\epsilon) &= +\sqrt{\epsilon} \l\sqrt{\frac{\alpha-m}{1+m}} - \epsilon\frac{1+\alpha}{2(1+m)} + \mathcal{O}(\epsilon^{\frac{3}{2}}) > 0,\\
%  \lambda_2(\epsilon) &= -\sqrt{\epsilon} \l\sqrt{\frac{\alpha-m}{1+m}} - \epsilon\frac{1+\alpha}{2(1+m)} + \mathcal{O}(\epsilon^{\frac{3}{2}}) < 0,\\
%  \lambda_3(\epsilon) &= -\epsilon(1+\alpha)<0.
% \end{align*}
%
...
%   \frac{1+m}{1+\alpha}\l \sqrt{-\frac{-\alpha+m}{1+m}} & -\frac{1+m}{1+\alpha}\l \sqrt{-\frac{-\alpha+m}{1+m}} & 0 \\
%   0 & 0 & 0 \\
...
%   1 & 1 & 1 \\
%   \frac{-\alpha+m}{1+\alpha}+\frac{1+\alpha}{2\l^2(1+m)}  & -\frac{-\alpha+m}{1+\alpha} +\frac{1+\alpha}{2\l^2(1+m)} & \frac{1+\alpha}{\l^2}
%  \end{pmatrix}
%  + \cdots,
% \end{align*}
...
% \begin{align*}
%  \begin{pmatrix}
%   \epsilon+ \frac{1+m}{1+\alpha}\lambda(\epsilon) \\ \epsilon \\ -\frac{\lambda(\epsilon)}{\l^2} \Big(\epsilon + \frac{1+m}{1+\alpha}\lambda(\epsilon)\Big)
%  \end{pmatrix}
% \end{align*}
% }
...
% \end{align*}
% and define constants $A_1 = (t_0 + \gamma_0)^{\ell_1}$, $A_2 = (t_0 + \gamma_0)^{\ell_2}$, $A_3 = (t_0 + \gamma_0)^{\ell_3}\exp\Big(n\l^2 \big(\frac{1+\alpha}{1+m}\big)^{\frac{1}{1+\alpha}}(t_0+\gamma_0)^{\frac{1+\alpha}{1+m}}\Big)$.
% 
...
%  \end{pmatrix} A_3(t+\gamma_0)^{-\ell_3}\exp\big(n\l^2 \big(\frac{1+\alpha}{1+m}\big)^{\frac{1}{1+\alpha}}(t+\gamma_0)^{\frac{1+\alpha}{1+m}}\big) = p_3,
% \end{align*}
% where
...
% Since the index condition holds for all $\ell=1,2,3$, $\varphi_\ell(\tau)$ $\ell=1,2,3$ comprise a fundamental matrix for $\tau\ge\tau_0$. However, because $\frac{1}{\tau}z_1(0)$ is the biggest among $\frac{1}{\tau}z_1(0)$, $\frac{1}{\tau}z_2(0)$, and $-n\l^2 + \frac{1}{\tau}\lambda_3'(0)$, in the applying the Theorem \ref{thm:CL} we find
