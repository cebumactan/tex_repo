...
\section{Formulation of Relative perturbations}
...
From the fourth equation in \eqref{eq:l-system}, we remove one of the variables. We choose to remove $\Theta$ and get three linear equations in the form 
$x'=A(\tau)x$, where
$x\triangleq (
...
%  \item saturation of the polynomial order is found as the oscillations in the perturbations get wilder.
% \end{enumerate}
...
&\lim_{\gamma_s \rightarrow \infty} C_3 x_3(\gamma_s) \exp\left(C_4n\l^2 \gamma_s^{\chi_4}\right)\gamma_s^{\chi_3} = p_3, \quad \text{for some $\chi_i$, $i=1,2,3,4$}.
 \end{aligned}
\end{equation}
Signs of $\chi_i$ indicate the stability in the respective mode. $p_i$, $i=1,2,3$ are the eigenvectors of $A_\infty$ which are
% \begin{align*}
...
$z_1(0)$ and $z_2(0)$ are defined in the following section and $C_j=C_j(\alpha,m,n)$, $j=1,2,3,4$ are complex constants. %As the third eigenvalue $\lambda_3(\tau)$ always has a leading order $-n\l$, we find
 Notably, $x_1$ and $x_2$ has the polynomial behavior; $\chi_4=\frac{1+\alpha}{1+m}>0$ and so $x_3$ decays exponentially regardeless of the sign of $\chi_3$.  Given above, we state our theorem: %what bifurcate are the polynomial growth or decay of $x_1$ and $x_2$.
...
\subsection{Asymptotic behaviors of linearized equations for viscous model}
...
\begin{align*}
 s_1(\tau)&=E(\tau) + F(\tau), \quad s_2(\tau)=E(\tau)-F(\tau), \quad s_3(\tau) = \begin{pmatrix} -m\\1\\ -\frac{m(1+\alpha)}{\tau\k^2} \end{pmatrix},\\
