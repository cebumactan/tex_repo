...
This article is devoted to the explanation of the onset of localizing instability and the dynamic formation of shear band in high strain-rate plasticity of metals. Our objective is to scrutinize the power law constitutive model in a validly designated range of exponents. The linearized problem is first treated, to find the role of the viscosity critical; we show Hadamard type instability in the inviscid law and Turing type instability in the viscous law. Typical difficulty of having non-autonomous terms in the linearized problem arises, which stems from having the uniform shearing solution, around which the system is linearized, not steady but evolutionay. Next, we begin nonlinear analysis. For the given nonlinear model we construct self-similar solutions that describe
...
First, high speed in the deformation alters the loading conditions from isothermal to that close to adiabatic. Having adiabatic conditions projects the belief that, at high strain rates, heat diffuses at a slower time scale than that of the deformation takes place so that effectively heat accumulates during the operating time. Under such conditions, since materials often thermally soften, in such a case where the thermal softening coutweighs the strain hardening the combined effect can lead to net softening response. This is to associate the rate dependent effect  in the corresponding model to the Hadamard instability.%(Indeed, experimental observations of shear bands are  typically associated  with strain softening response -- past a critical strain -- of the measured stress-strain curve \cite{CDHS}.)
...
In this review work we seek rather wide range of understanding regarding the shear bands formation of thermoviscoplastic power law materials. We carry out linear and nonlinear stability of the uniform shear and aim to compute numerical solutions as well. We listed the tasks of this work:
\begin{itemize}
\item To define a notion of stability (or instability) for the uniform shear; as this base solution is evolutionay as addressed, this task becomes nontrivial due to the the time dependent non-autonomous nature of the system.
\item To derive quantitative criteria for stability (or instability) for the linearized problem and to give a complete analysis of linear stability by the criteria. This gives a mathematical accounts pinpointing the onset of localization. 
...
% \end{aligned}
% \end{equation}
In the uniform shearing motion, strain $\gamma_s$ and the temperature $\theta_s$ as well as time $t$ are regarded as interchangeable measures of time elapsed. % and each can serve as an independent variable. 
We find having $\gamma_s$ as an independent variable neat. We define quantities homogeneous in $\gamma _s$, which are leading orders of the uniform shearing solutions,
...
Then $\Gamma\big(\tau,x\big)$, $\Theta\big(\tau,x\big)$, $U\big(\tau,x\big)$, $\Sigma\big(\tau,x\big)$ are defined by relations $\tau=\tau(\gamma_s)$ and
\begin{equation}
 \begin{aligned}
  \gamma(\gamma_s,x) &= \gamma^*_s\Gamma\big(\tau(\gamma_s),x\big), \quad \theta(\gamma_s,x) = \theta^*_s\Theta\big(\tau(\gamma_s),x\big), \quad u(\gamma_s,x)=U\big(\tau(\gamma_s),x\big),\\
  \sigma(\gamma_s,x) &= \sigma^*_s\Sigma\big(\tau(\gamma_s),x\big) \quad \Longrightarrow \quad \Sigma = \Theta^{-\alpha}\Gamma^m U^n.
 \end{aligned}
\end{equation}
New variables of relative perturbations compares the growths homogeneous in $\gamma_s$ of variables to those of uniform shearing. In this formulation, the uniform shearing solutions are mapped to (asymptotically) constant states. The price is to have non-autonomous terms in the system as in the below. %In an effort of having non-autonomous coefficient as little as possible,  
...
%We find a few mathematical issues in \cite{FM} unresolved and the general case of $3\times3$ system for $(U,\Gamma,\Theta)$ were not fully covered. We conducted the analysis in more systematic way but to reach roughly the same conclusion.
...
From the fourth equation in \eqref{eq:l-system}, we remove one of the variable. We choose to remove $\Theta$ and get three linear equations in the form 
$x'=A(\tau)x$, where
$x\triangleq (
...
% \begin{enumerate}
%  \item $n>0$, $\kappa=0$ or linear stability of the viscous adiabatic cases: 
%  
%  We derive the instability criterion $q=-\alpha+m+n$. We show that the uniform shearing solution is $(i)$ stable if $q>0$; $(ii)$ unstable if $q<0$. Moreover, we find the Turing type instability in the unstable regime:
% \begin{enumerate}
%  \item the asymptotic growths are limited to be of polynomial order;
%  \item saturation of the polynomial order are found as the oscillations in the perturbations get wilder.
% \end{enumerate}
%   \item $n=0$, $\kappa=0$ or linear stability of the inviscid adiabatic cases: 
%   
%   For this we focus on contrasting differences in the unstable regime $q=-\alpha+m<0$. If $q=-\alpha+m<0$, the uniform shearing solution is again unstable but is of Hadamard type; the asymptotic growths are exponential and no saturation in the exponent is observed; exponent diverges as the oscillations in the perturbations get wilder.
% \end{enumerate}
...
Let us be more specific with the following exposition. For each $\ell\ge 1$ the three linearly independent modes $x_1(\gamma_s)$, $x_2(\gamma_s)$, and $x_3(\gamma_s)$ %$x_1\circ\tau(\gamma_s)$, $x_2\circ\tau(\gamma_s)$, and $x_3\circ\tau(\gamma_s)$ 
always are found, comprising a fundamental matrix, and this gives us the complete characterization of the behaviors of the system. Their precise asymptotic behaviors will be shown:
\begin{equation}
\begin{aligned}
...
&\lim_{\gamma_s \rightarrow \infty} C_3 x_3(\gamma_s) \exp\left(C_4n\l^2 \gamma_s^{\chi_4}\right)\gamma_s^{\chi_3} = p_3, \quad \text{for some $\chi_i$, $i=1,2,3,4$}.
 \end{aligned}
\end{equation}
...
\begin{align*}
 ...
 \end{align*}
$z_1(0)$ and $z_2(0)$ are defined in the following section and $C_j=C_j(\alpha,m,n)$, $j=1,2,3,4$ are complex constants. %As the third eigenvalue $\lambda_3(\tau)$ always has a leading order $-n\l$, we find
Signs of $\chi_i$ indicate the stability in the respective mode. Notably, $x_1$ and $x_2$ has the polynomial behavior; $\chi_4=\frac{1+\alpha}{1+m}>0$ in all cases we discuss and so $x_3$ decays exponentially regardeless of the sign of $\chi_3$.  Given above, we state our theorem: %what bifurcate are the polynomial growth or decay of $x_1$ and $x_2$.
...
\subsection{Asymptotic behaviors of viscous model}
...
As the order $\frac{1}{\tau}$ perturbation is explicitly given, straightforward calculation shows that eigenvectors for finite $\tau$ can be chosen
\begin{align*}
...
% \begin{theorem}\label{thm:2} Suppose $q=-\alpha+m<0$, then the uniform shear is unstable: For each $\ell\ge 1$, there is a growing mode $x(\gamma_s)$ of the system $\eqref{eq:l-system}_\ell$, $|x(\gamma_s)| \ge C\exp\left(C'\l \gamma_s^{\frac{1+m}{2(1+\alpha)}}\right)$ for $\gamma_s\ge\gamma_{s0}$ large enough for some positive constant $C(\alpha,m,n)$ and $C'(\alpha,m,n)$. Asymptotic stability of the uniform shear bifurcates by the sign of $q=-\alpha+m$:
%  \begin{enumerate}
%   \item If $q=-\alpha+m=0$, $\alpha>0$ then the uniform shear is marginally stable: For all $\ell\ge 1$ and for any solution $x(\gamma_s)$ of the system $\eqref{eq:l-system}_\ell$, $|x(\gamma_s)| < C$.
%   \item If $q=-\alpha+m>0$, then the uniform shear is stable: For all $\ell\ge 1$ and for any solution $x(\gamma_s)$ of the system $\eqref{eq:l-system}_\ell$, $|x(\gamma_s)| = \mathcal{O}\big(\gamma_s^{\chi}\big)$ as $t \rightarrow \infty$, $\chi = \max \left\{-1, -(1+m)\right\}$.
%   \item 
...
2. workout n=0 instability

3. add paragraphs on $n=0$ case phase space analysis in section 4

4. streamline the Appendix

5. proofreading

6. work through section 3 to include n=0 unstable case. 

7. work through section 4

8. work through introduction
