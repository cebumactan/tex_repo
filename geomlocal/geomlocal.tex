...
\usepackage{color,mathtools,psfrag}
...
\newcommand{\TheTitle}{Existence of localizing solutions in plasticity \\ via geometric singular perturbation theory} 
\newcommand{\TheAuthors}{Min-Gi Lee and Athanasios Tzavaras}

% Sets running headers as well as PDF title and authors
\headers{Localization and Singular Perturbations}{\TheAuthors}

% Title. If the supplement option is on, then "Supplementary Material"
% is automatically inserted before the title.
\title{{\TheTitle}\thanks{Research supported by King Abdullah University of Science and Technology (KAUST).}}

% Authors: full names plus addresses.
\author{
  Min-Gi Lee\thanks{Computer, Electrical, Mathematical Sciences \& Engineering Division, King Abdullah University of Science and Technology (KAUST), Thuwal, Saudi Arabia. (\email{mingi.lee@kaust.edu.sa})}  % \ \thanks{Corresponding author : \texttt{mingi.lee@kaust.edu.sa}}
  \and
  Athanasios Tzavaras\footnotemark[2] %
}
...
The system \cref{intro-system} is a model from viscoplasticity that serves as a simplified model to understand the problem of 
...
\cref{intro-system} admits a class of solutions,  that are valid for any values of the parameters $m$ and $n$ and describe uniform shearing 
\begin{align}
\label{intro-uss}
...
\label{intro-ssdata}
 ...
 \end{align}
where  $\bU(\xi) = \bV_\xi(\xi)$ and $\bG_0$ and $\bU_0$ are given positive parameters. As seen from $\cref{intro-sseqns}_1$ at $\xi=0$, 
$$
...
Remarkably, the invariance properties of the system \cref{intro-sseqns} allows the  de-singularization of the system \cref{intro-sseqns}
(see \cref{eq:tvars} and \cref{eq:auto0}). Furthermore, a nonlinear change of variables (see \cref{eq:ratios}) leads to reformulating the problem
...
\end{align}}
Moreover, the question of existence of a solution $\big(\bV,\bG)$ to \cref{intro-sseqns}, \eqref{intro-ssdata} is reformulated 
to that of the construction of a suitable heteroclinic orbit to \cref{intro-pqrsystem}. The difficulty with the construction of
such heteroclinics originates from the dimensionality of the system \cref{intro-pqrsystem}.
...
As $n$ is a small parameter,  the system \cref{intro-pqrsystem} admits both fast and slow time scales. Problems with multiple time scales are habitually found 
...
to the system \cref{intro-system} is outlined in section 6 (see \cref{eq:sssol}), where various properties, such as the  range of parameters 
...
$$
respectively. 
\cref{eq:system0} consists of momentum conservation and kinematic compatibility, where $v$ is the velocity in the shearing direction, 
...
the system \cref{eq:system0} is elliptic in the $t$-direction. Then the initial value problem exhibits
...
The model \cref{eq:system0}-\cref{eq:constitution1} admits the uniform shearing solutions, 
...
to unstable modes that grow faster than the uniform shear \cref{eq:uss}. 
...
In this section, we study a family of focusing self-similar solutions for \cref{eq:system} in the parameter regime $0 < n < m \le 1$.
...
We begin by investigating the scale invariance properties of the system \cref{eq:system}. For a given $(\gamma,v)$, we define $(\gamma_\rho$, $v_\rho)$ and the new independent variables $y$ and $s$ such that
...
\end{aligned}
\end{equation}
Due to the choice $\lambda>0$, this transformation makes the profile narrower and higher for $\rho$ large. This is of course in accordance with our goal, constructing solutions that localize the initial profile. A simple calculation shows that \cref{eq:system} is invariant under \cref{eq:scale_inv} if the exponents $a$ and $b$ are selected as 
...
\end{aligned}
\end{equation}
with $\xi=(t+1)^ \lambda x$ is the similarity variable. Substitution into \cref{eq:system} gives a system of ordinary differential equations
...
Given a  smooth solution of \cref{eq:sseqns}, \eqref{ssdata} for parameters $\lambda$, $m$ and $n$, it will generate the profile  of a solution to \cref{eq:system} that localizes 
...
% A family Solutions for a given $m$ and $n$ that localize in a self-similar way at localizing rate $\lambda$ will be attained by achieving solutions of system \cref{eq:sseqns}.% for each admissible $m$, $n$ and $\lambda$.

The system \cref{eq:sseqns} is non-autonomous and singular at $\xi=0$. 
...
In the present case, it is possible to de-singularize \cref{eq:sseqns}, turning it into an autonomous system of three differential equations. 
In a second step the problem is turned into the construction of a heteroclinic orbit for an equivalent system. 
The existence of the heteroclinic orbit is achieved in \cref{sec:proof} by employing geometric singular perturbation theory.
...
The system \cref{eq:sseqns} has its own scale invariance. For a given $\big(\bG(\xi), \bV(\xi)\big)$, we define
\begin{equation}
 \bG_A(\xi) = A^\alpha \bG(A\xi), \quad \bV_A(\xi) = A^\beta \bG(A\xi).
\end{equation}
The exponents $\alpha$ and $\beta$ that make \cref{eq:sseqns} invariant are
\begin{equation}
...
% $
Substitution into \cref{eq:sseqns} gives the system for the residual variables $(\tg,\tv,\tu,\ts)$
\begin{equation}
 \begin{split}
...
Using \cref{eq:sseqns}, heuristic calculations can be carried out to find which three pairs of quantities are expected to share the same asymptotic leading order. 
...
%  \end{split}
\end{equation}
Substitution into \cref{eq:auto0} leads to the equivalent system
...
% \cref{eq:pqrsystem} has $4$ equilibrium points and possibly their stable and unstable manifolds intersect producing many connections. On the other hands, we have controls in the asymptotic behavior of the relevant solutions on both ends of $\eta \rightarrow \pm \infty.$ As $\eta \rightarrow -\infty$ the regularity conditions \cref{eq:continuity} imposed at $\xi=0$ characterizes where the orbit should end. As $\eta \rightarrow \infty$, physically relevant expectation gives the characterization. 

In this section, we carry out the following steps: 
\begin{enumerate}
 \item[(i)] In \cref{sec:equi}, we find the equilibria $M_i^{\lambda,m,n}$, $i=0,1,2,3$, of \cref{eq:pqrsystem} and compute their local eigenstructure.
 \item[(ii)] In \cref{sec:char}, we single out a heteroclinic orbit having the expected behavior as $\eta \rightarrow \pm \infty$. 
% \item[(iii)] in \cref{sec:summarize}, we make a connection from the singled out heteroclinic orbit to the family of focusing self-similar solutions.
\end{enumerate}

\subsection{Equilibria and linear stability} \label{sec:equi}
The system \cref{eq:pqrsystem} has four equilibrium points in the first octant of the phase space. The four equilibrium points are
\begin{align*}
 M_0^{ \lambda,m,n} 
%      &= 
%        \begin{pmatrix}
...
%        \end{pmatrix}
     = \begin{pmatrix} 
       ...
       \end{pmatrix},    
\end{align*}
where $a$ is the exponent in  \cref{eq:s_inv} and 
\begin{equation}
\label{defc}
...
%   }
  \caption{Equilibria of the $(p,q,r)$-system and the associated linearized vector fields for $\lambda$ satisfying \cref{eq:upperbdd} and for $\mu_{31}>0$. }
\end{figure}
...
increasing function of time at any spatial point. For example, the strain of the uniform shearing solutions \cref{eq:uss} grows linearly in time. 
...
The boundary conditions \cref{eq:continuity} provide the desired behavior as $\eta \to -\infty$. 

\begin{proposition} \label{lem:alpha} Let $\big(\bG(\xi),\bV(\xi)\big)$ be a smooth solution of \cref{eq:sseqns} 
with boundary conditions \cref{eq:continuity}, and let $\big(p(\eta),q(\eta),r(\eta)\big)$ be the associated orbit obtained via the
transformations \cref{eq:tvars} and \cref{eq:ratios}. Then
\begin{align}
...
\cref{eq:asym_alpha} indicates that $\bG(0)$, the constant factor of the self-similar strain profile is responsible for fixing the shift $\eta_0$. Let us be precise on this procedure.
...
\end{equation}
where we exploited $\frac{\bU(0)}{\bG(0)} = a$. \cref{eq:upperbdd} pinpoints the range  
...
in the range  \cref{eq:upperbdd2}. 
\end{remark}
...
near $M_1$ on $G$.  Similarly to \cref{eq:expansion}, we have
...
\begin{proposition}
 Let $\big(\bG(\xi),\bV(\xi),\bS(\xi),\bU(\xi)\big)$ be a smooth self-similar profile satisfying \cref{eq:continuity} and let $\varphi^\star(\eta)=\big(p(\eta),q(\eta),r(\eta)\big)$ be the associated variables defined by \cref{eq:tvars} and \cref{eq:ratios}. Suppose $\varphi^\star(\eta)$ is the heteroclinic orbit of \cref{eq:pqrsystem} that connects $M_0$ to $M_1$. Then, 
 \begin{enumerate}
...
\end{proposition}
\begin{proof}
  $(i)$ and $(ii)$ were verified in the proof of  \cref{lem:alpha}. To show $(iii)$, the asymptotic behavior of $\varphi^\star(\eta)$ in a neighborhood of $M_1$ is investigated. The asymptotic behavior of a nonlinear problem near the hyperbolic equilibrium point $M_1$ is determined by the associated linearized problem. 
As the orbit $\varphi^\star(\eta)$ lies in the stable manifold of $M_1$,  its asymptotic behavior  in the neighborhood of $M_1$ is expressed by \cref{eq:expansion2_special} for the case $m-n= \frac{1}{2}$ and $\lambda \ne 1-m$ and by \cref{eq:expansion2} for the rest of cases. We first focus on the latter.

For the orbit $\varphi^\star(\eta)$, one can further exclude the possibility that the coefficient $\kappa'_2 = 0$ in \cref{eq:expansion2}.
Indeed,  the plane $p=0$ is invariant for the dynamical system \cref{eq:pqrsystem}, and the corresponding orbits on the plane $p=0$ can be calculated. 
...
\end{equation*}
as $\eta \rightarrow \infty$ and then setting  $\xi = e^\eta$ and \cref{eq:tvars} provides $(iii)$-(b).

For the special case when $m-n= \frac{1}{2}$ and $\lambda \ne 1-m$, with same reasoning $\kappa_2'\ne0$ in \cref{eq:expansion2_special} and the $p$-component of $\vec{X}_{11}$ vanishes but we have
...
In the preceding section, we identified the heteroclinic of \cref{eq:pqrsystem} on the hypothesized surface $G = W^u(M_0) \cap W^s(M_1)$. We are now in a position to apply geometric singular perturbation theory to achieve the surface and the heteroclinic orbit. Our goal is to prove the following theorem.
...
\end{align}
 For each $(\lambda,m,0) \in \Lambda$, there is $n_0( \lambda,m)$, such that for $n \in (0, n_0)$, $(\lambda,m,n) \in \Lambda$ and the system \cref{eq:pqrsystem} admits a heteroclinic orbit joining equilibrium $M_0^{\lambda,m,n}$ to equilibrium $M_1^{\lambda,m,n}$ with the following property
 \begin{equation} 
     e^{-2\eta}\left[\begin{pmatrix}
   p(\eta) \\ q(\eta) \\ r(\eta)
  \end{pmatrix}
...
\end{equation}
In view of \cref{eq:level}, the contour lines in the $pq$-plane sweep out the first quadrant from the origin. See \cref{fig:contour}. More precisely, the contour line passes the origin when $\underbar{r}=a^{ \lambda,m,0}$ at the same time as its lift in the $pqr$-space passes the equilibrium $M_0^{ \lambda,m,0}$. As $\underbar{r}$ decreases, the contour line intersects the $p$ and $q$ axes and becomes steeper. When $\underbar{r}$ reaches $c^{ \lambda,m,0}$, the contour line passes $(0,1)$ at the same time as its lift passes $M_1^{ \lambda,m,0}$. $\underbar{r}$ then further decreases to $0$. 

Note that the inequality \cref{eq:a4} implies $c^{ \lambda,m,0}>0$. We let $T$ be the triangle enclosed by the $p$-axis, $q$-axis and the one contour line of \cref{eq:level} with $0<\underbar{r} < c^{\lambda,m,0}$. We choose $D \supset\supset T$ whose compact closure $\bar{D}$ is strictly away from $r=0$ plane. The critical manifold for each $\lambda$ and $m$ is defined by 
\begin{equation}
...
The marginal case $m=1$ provides closer detail. If one substitutes $m = 1$, $n = 0$, $h^{\lambda,1,0}(p,q)$ in the first two equations of \cref{eq:pqrsystem}, the resulting system can be explicitly solved. The general solution on the graph is a family of parabolae $p=kq^2$ and $r=h^{\lambda,1,0}(p,q)$. This includes the two extremes $p=0$ and $q=0$, where $k$ takes $0$ and $\infty$ respectively. See \cref{fig:hn0m1}. We focus on discussing two points: 1) In an effort to apprehend the flow of the rest of cases, we remark a few features for this marginal case, which in turn persist under the perturbation; and 2) we report features that do not persist too. These features do not play any role in our study, but this degenerate case is described here for clarity.

We address the first point. Emanating from $M_0^{ \lambda,1,0}$
in \cref{fig:hn0m1_b} is a family of parabolae. Our interested direction $\vec{X}_{02}$ and the other $\vec{X}_{01}$ are indicated near $M_0^{ \lambda,1,0}$ by a dotted arrow. The family of parabolae is manifesting the fact that orbit curves meet $M_0^{ \lambda,1,0}$ tangentially to $\vec{X}_{01}$; one exception is the degenerate straight line that emanates in $\vec{X}_{02}$, which is depicted as the green one in \cref{fig:hn0m1}, the target orbit. Another observation from the $pq$-plane is that the flow in the first quadrant far away from the origin is {\it inwards}. More precisely, as illustrated in \cref{fig:hn0m1_b}, whenever $0<\underbar{r} < 1 = c^{\lambda,1,0}$ the flow on the contour line $\underbar{r} = h^{\lambda,1,0}$ is inwards. We make use of this observation in the proof of \cref{sec:proof_proof}.

Now, we describe the second point. The crucial difference is that $M_1^{\lambda,1,0}$ is replaced by a line of equilibria $h^{\lambda,1,0}(p,q) = c^{\lambda,1,0}=1$, which is the red line in \cref{fig:hn0m1}. As a result, each of the parabolae emanated from $M_0^{\lambda,1,0}$ lands at a point among these equilibria. $\vec{X}_{02}$ lies in the plane $q=0$ distinctively from all other cases and the target orbit in particular lands at the $q$-intercept of the line of equilibria. To compare this observation to the statement of \cref{thm:1}, the target orbit does not connect $M_0^{ \lambda,1,0}$ to $M_1^{ \lambda,1,0}$ but to this $q$-intercept. This observation does not spoil our proof in \cref{sec:proof_proof} because we assert the persistence of the critical manifold not the target orbit.
\begin{figure}[h]
  \centering
  \subfloat[phase space]{
    \psfrag{p}{$p$}
...
\end{step}
By \cref{lem:normal_hyper}, there exists $n_0$, such that for $n \in [0, n_0)$, locally invariant manifold $G^{\lambda,m,n}$ with respect to \cref{eq:pqr_fast} exists. Moreover,   $G^{\lambda,m,n}$ is again given by the graph $(p,q,h^{\lambda,m,n}(p,q))$ on $\bar{D}$. The condition that $G^{\lambda,m,n}$ is disjoint from $r=0$ plane for all $n \in [0, n_0)$ must persist by making $n_0$ smaller if necessary. In addition, $n_0$ is chosen in the valid range of inequalities \cref{eq:a3} and \cref{eq:a4}.%$h^{\lambda,m,n}(p,q)>0$ in the domain of definition for all $0\le n\le n_2$ has to persist by taking $K^{\lambda,m,0}$ and $n_2$ appropriately.  %On this surface, $r$ evolves such a way staying in the surface.

After achieving $h^{\lambda,m,n}(p,q)$, substitution of the function in place of $r$ in system \cref{eq:pqrsystem} leads to  the reduced systems that are parametrized by $\lambda$, $m$, and $n\in[0,n_0)$:
% {\small
...
% \begin{step}
%  $(0,0)$, the projection on the $pq$-plane of $M_0^{\lambda,m,n}$, is an unstable node with respect to \cref{eq:reduced}. $(0,1)$, that of $M_1^{\lambda,m,n}$, is a stable node with respect to \cref{eq:reduced}.
% \end{step}
...
\medskip
\begin{step}
 $T$ is positively invariant under the flow \cref{eq:reduced} if $n$ is sufficiently small.
\end{step}
...
\begin{figure}[h]
  ...
  \subfloat[$pq$-plane]{
    ...
    \caption{Schematically drawn sketches of the perturbed invariant manifold and the flow. The triangle $T$ was determined by the contour line of the graph $\underbar{r}=h^0(p,q)$ but $T$ is positively invariant under the flow \cref{eq:reduced} for all $n \in [0, n_0)$.}
\end{figure}
...
%     \caption{The triangle $T$ is determined by the contour line of the graph $\underbar{r}=h^0(p,q)$. However, $T$ is positively invariant under the flow \cref{eq:reduced} for all $n \in [0, n_0)$. }
% \end{figure}
...
\cref{thm:1} determines the orbit curve, and the translation factor $\eta_0$ fixes the one heteroclinic orbit. As stated earlier, $\big(\bG(0),\bU(0)\big)$ determine $\eta_0$ and $\lambda$  by \cref{eq:lambda_eta}. In summary, for each $\big(\bG(0),\bU(0)\big)$ such that
\begin{equation*}
 \frac{2-n}{1+m-n} \;<\; \frac{\bU(0)}{\bG(0)} \;<\; \frac{2-n}{1-m+n} \, ,
\end{equation*}
and for material parameters $m$ and $n$ this procedure gives rise to a solution of \cref{eq:system}. By tracing back the nonlinear transformations \cref{eq:ratios}, \cref{eq:tvars}, and \cref{eq:ss}
\begin{align}
 \begin{aligned}
...
From \cref{eq:ss_asymp0} and \cref{eq:ss_asymp1} we infer that $\bG$ and $\bU$ peak at the origin and decay at the order $\xi^{ -\frac{1}{m-n}}$ as $\xi \rightarrow \infty$. 
...
due to the multiplier polynomials of $t$ in \cref{eq:sssol}, the heights of the profiles increase (or decrease). The actual rate of growth or decay  at a fixed point $x\ne0$ is calculated by taking the shrinking effect into account, contrasting the behavior near the origin to the rest of the points.
...
\begin{equation} \label{eq:flow0}
 ...
 \end{split}
\end{equation}
Here, $x\in \mathbb{R}^m$ is called a fast variable and $y\in \mathbb{R}^n$ a slow variable. We assume the vector field of \cref{eq:flow} has a definition for $(x,y)\in U\subset\mathbb{R}^{m+n}$ an open set and for $\epsilon\in I$, the interval containing $0$. Let 
...
\begin{definition}[Normally hyperbolic invariant manifold in $C_0$ to \cref{eq:flow0}] \label{def:nhim}
...
\begin{theorem}[Graph version, theorem 2 in \cite{jones_geometric_1995}] \label{thm:fenichel}
 Under the assumptions (H1),\\ (H2), and (H3), if $\epsilon>0$ is sufficiently small, there is a function $h^\epsilon(y)$, defined on $\bar{D}$, so that the graph $M_\epsilon = \{(x,y)~|~ x=h^\epsilon(y)\}$ is locally invariant to \cref{eq:flow}. Moreover, $h^\epsilon$ is $C^r(\bar{D})$ for any $r<+\infty$, jointly in $y$ and $\epsilon$.
\end{theorem}
